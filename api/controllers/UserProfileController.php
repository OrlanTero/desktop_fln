...
class UserProfileController
{
    ...
    public function updateProfile($userId, $data)
    {
        ...
        if (isset($data->department))
            $this->userProfile->department = $data->department;
        if (isset($data->skills))
            $this->userProfile->skills = $data->skills;
        if (isset($data->social_links))
            $this->userProfile->social_links = $data->social_links;
    ...
    public function uploadSignature($userId, $file)
    {
        ...
        if (!isset($file) || !is_array($file)) {
            return array(
                "success" => false,
                "message" => "No signature uploaded or invalid file data"
            );
        }

        // Check if file was uploaded without errors
        if (isset($file["error"]) && $file["error"] !== 0) {
            $errorMessage = "Error uploading signature";
            switch ($file["error"]) {
                case UPLOAD_ERR_INI_SIZE:
                    $errorMessage = "The uploaded signature exceeds the upload_max_filesize directive in php.ini";
                    break;
                case UPLOAD_ERR_FORM_SIZE:
                    $errorMessage = "The uploaded signature exceeds the MAX_FILE_SIZE directive that was specified in the HTML form";
                    break;
                case UPLOAD_ERR_PARTIAL:
                    $errorMessage = "The uploaded signature was only partially uploaded";
                    break;
                case UPLOAD_ERR_NO_FILE:
                    $errorMessage = "No signature was uploaded";
                    break;
                case UPLOAD_ERR_NO_TMP_DIR:
                    $errorMessage = "Missing a temporary folder";
                    break;
                case UPLOAD_ERR_CANT_WRITE:
                    $errorMessage = "Failed to write signature to disk";
                    break;
                case UPLOAD_ERR_EXTENSION:
                    $errorMessage = "A PHP extension stopped the signature upload";
                    break;
                default:
                    $errorMessage = "Unknown upload error: " . $file["error"];
            }
        ...
        if (!isset($file["name"]) || !isset($file["tmp_name"]) || empty($file["name"]) || empty($file["tmp_name"])) {
            return array(
                "success" => false,
                "message" => "Invalid signature file data"
            );
        }
        ...
        // Generate unique filename
        $fileExtension = pathinfo($file["name"], PATHINFO_EXTENSION);
        $newFilename = "sig_" . $userId . "_" . time() . "." . $fileExtension;
        $targetFile = $uploadDir . $newFilename;
        ...
        if (!in_array(strtolower($fileExtension), $allowedTypes)) {
            return array(
                "success" => false,
                "message" => "Only JPG, JPEG, PNG & GIF files are allowed for signatures"
            );
        }
        ...
        if ($file["size"] > 2000000) {
            return array(
                "success" => false,
                "message" => "Signature file is too large. Maximum size is 2MB"
            );
        }
        ...
        if (move_uploaded_file($file["tmp_name"], $targetFile)) {
            ...
            $signatureUrl = "/uploads/signatures/" . $newFilename;

            // Update the user record with the new signature URL
            $query = "UPDATE users SET signature_url = :signature_url WHERE id = :id";
            $stmt = $this->db->prepare($query);

            // Sanitize and bind
            $sanitizedSignatureUrl = htmlspecialchars(strip_tags($signatureUrl));
            $sanitizedUserId = htmlspecialchars(strip_tags($userId));

            $stmt->bindParam(":signature_url", $sanitizedSignatureUrl);
            $stmt->bindParam(":id", $sanitizedUserId);
            ...
            if ($stmt->execute()) {
                ...
                return array(
                    "success" => false,
                    "message" => "Failed to update signature URL in database"
                );
            }
        } else {
            return array(
                "success" => false,
                "message" => "Failed to upload signature file. Check file permissions."
            );
        }
    }
}